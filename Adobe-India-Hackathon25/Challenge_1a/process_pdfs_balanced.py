#!/usr/bin/env python3
"""
Adobe Hackathon 2025 - BALANCED PDF Processing System
Optimized heading detection with balanced accuracy and precision
"""
...
class BalancedPDFProcessor:
    """Balanced PDF processor with optimized heading detection"""
    ...
    def __init__(self, font_size_threshold: float = 1.15, min_heading_length: int = 2, use_ml_models: bool = False):
        ...
        # BALANCED: Optimized scoring weights for better precision/recall balance
        self.scoring_weights = {
            'font_size': 0.40,   # PRIMARY: Font size is most reliable indicator
            'content': 0.35,     # SECONDARY: Enhanced semantic content patterns
            'formatting': 0.15,  # TERTIARY: Bold/italic with better filtering
            'position': 0.10     # MINIMAL: Position for document structure
        }
        
        # BALANCED: Enhanced keyword patterns with better coverage
        self.heading_keywords = [
            ...
            'pathway options', 'regular pathway', 'distinction pathway', 'parsippany', 'stem pathways'
        ]
        
        # BALANCED: Focused false positive patterns (less aggressive)
        self.false_positive_patterns = [
            # Clear UI Elements and Instructions
            ...
            r'^[a-z0-9]{12,}$',  # Very long alphanumeric strings
    ...
    def _is_definitely_not_heading(self, text: str) -> bool:
        """BALANCED: Focused false positive filtering"""
        ...
        # Filter very long text (likely paragraphs) - more lenient
        if len(text) > 150:
            return True
        
        # Filter text with too many words (likely sentences) - more lenient
    ...
    def score_heading_candidate(self, span: TextSpan, font_stats: Dict[str, any], 
        ...
        """BALANCED: Enhanced multi-factor scoring for heading detection"""
        text = span.text.strip()
        
        # Early filtering - more lenient
        ...
        # 1. Font Size Score (40% weight) - BALANCED
        ...
        # 2. Content Pattern Score (35% weight) - BALANCED
        content_score = 0.0
        ...
        if self._looks_like_heading_content(text):
            ...
            content_score = 0.6  # Short phrases - more lenient
        elif text.isupper() and 2 <= len(text.split()) <= 8:
            content_score = 0.7  # ALL CAPS headings - more lenient
        
        score += content_score * self.scoring_weights['content']
        
        # 3. Formatting Score (15% weight) - BALANCED
        format_score = 0.0
        if span.is_bold:
            format_score += 0.6
        if span.is_italic:
        ...
        # 4. Position Score (10% weight) - BALANCED
        if page_height > 0:
    ...
    def _looks_like_heading_content(self, text: str) -> bool:
        ...
        heading_patterns = [
            ...
            'content', 'navigation', 'reading', 'parsippany', 'stem pathways'
        ]
        
        if any(pattern in text_lower for pattern in heading_patterns):
            return True
        
        # ALL CAPS short phrases (likely headings) - more lenient
    ...
    def _appears_standalone_text(self, text: str) -> bool:
        ...
        # Not too long (likely paragraph) - more lenient
    ...
    def classify_heading_level(self, score: float, span: TextSpan, font_stats: Dict[str, any]) -> Optional[str]:
        """BALANCED: Enhanced heading level classification"""
        if score < 0.25:  # Lower threshold for better recall
            return None
    ...
    def detect_title(self, pdf_path: str, spans: List[TextSpan]) -> str:
        """BALANCED: Enhanced document title detection"""
        try:
    ...
    def detect_headings(self, pdf_path: str) -> List[Dict]:
        """BALANCED: Enhanced heading detection with better filtering"""
        try:
...
def process_pdfs():
    ...
    logger.info(f"🚀 Starting BALANCED PDF Processing for {len(pdf_files)} files")
    print("Starting balanced PDF processing...")
    
    processor = BalancedPDFProcessor()
    ...
    print("Balanced PDF processing completed!")
