...
ls -la output/*_enhanced.json | wc -l
```
...
└── DEBUGGING_AND_IMPROVEMENT_GUIDE.md  # Technical details
```
...
- **file01.pdf (LTC Form)**: 0.4s - 12 headings detected
- **file02.pdf (Technical)**: 3.2s - 76 headings detected  
- **file03.pdf (Business)**: 5.3s - 129 headings detected
- **file04.pdf (Education)**: 0.6s - 8 headings detected
- **file05.pdf (Event)**: 0.4s - 6 headings detected
- **sample.pdf (LaTeX)**: 1.9s - 43 headings detected
- **pdftest1.pdf (Complex)**: 5.3s - 97 headings detected
- **French 1A.pdf**: 2.8s - 117 headings detected

**Total Processing Time**: ~20 seconds for 8 diverse PDFs
...
✅ **Processing Time**: <10 seconds per document (typically 0.4-5.3s)  
...
- **Line-level Merging**: Combines fragmented text spans for coherent headings
- **Multi-line Support**: Handles table cells and text boxes spanning multiple lines
- **Font Analysis**: Statistical analysis of font sizes and styles
- **Position Awareness**: Spatial analysis for proper hierarchy detection

### Machine Learning Integration
- **Fine-tuned BERT**: Custom binary classifier for heading vs non-heading detection
- **Semantic Similarity**: Enhanced sentence transformer for context understanding  
- **Ensemble Scoring**: Combines multiple ML approaches for higher accuracy
- **Confidence Boosting**: Increases confidence when models agree

### Robust Error Handling
- **Fallback Processing**: Graceful degradation when ML models fail
- **Format Compatibility**: Handles diverse PDF creation tools and formats
- **Memory Management**: Efficient processing of large documents
- **Performance Monitoring**: Detailed timing and accuracy metrics
